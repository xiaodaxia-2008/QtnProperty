...
public:
	...
	QTextStream &newLine(int indentOffset = 0)
	{
		...
		return *this << endl
					 << QString(TAB_LEN * (m_indent + indentOffset), ' ');
#else
		return *this << Qt::endl
					 << QString(TAB_LEN * (m_indent + indentOffset), ' ');
#endif
	...
	void pushWrapperLines(
		const QString &firstLine, const QString &lastLine, int indentOffset = 0)
	{
...
private:
	void generate(TextStreamIndent &s) const
	{
		s.newLine() << "#include " << name;
	}
...
struct PropertySetCode
	: public Code
	, public PropertySetMemberCode
{
